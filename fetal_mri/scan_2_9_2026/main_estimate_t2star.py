"""
Coregister multi-echo fetal T2-weighted MRI stacks using FSL flirt,
then estimate T2* map via mono-exponential fitting.

Model: S(TE) = S0 * exp(-TE / T2*)

Pipeline:
  1. Register TE=140, 181, 272 images to the TE=98 reference using FSL flirt
     (6-DOF rigid body, correlation ratio cost, sinc interpolation).
  2. Voxel-wise log-linear fit → T2* and S0 initial estimates.
  3. Non-linear least-squares refinement (Levenberg-Marquardt).
  4. Save T2*, S0, R2* maps as NIfTI.
...
# Echo times in milliseconds (order must match input_files)
TE_values = np.array([98.0, 140.0, 181.0, 272.0])  # ms

# Input images — one sagittal stack per TE from scan_12_03_2025
data_dir = '/deneb_disk/disc_mri/scan_12_03_2025/nifti_rot'
input_files = [
    os.path.join(data_dir, 'p10_t2_haste_sag_head_te98_p.nii.gz'),
    os.path.join(data_dir, 'p11_t2_haste_sag_head_te140_p.nii.gz'),
    os.path.join(data_dir, 'p12_t2_haste_sag_head_te181_p.nii.gz'),
    os.path.join(data_dir, 'p13_t2_haste_sag_head_te272_p.nii.gz'),
]

# The first image (TE=98) is the reference for coregistration
reference_idx = 0

# Output directory
output_dir = '/deneb_disk/disc_mri/scan_12_03_2025/t2star_output'
...
def main():
    ...
    # --- Step 0: Coregister with FSL flirt ---
    print("\nStep 0: Coregistering images with FSL flirt ...")
