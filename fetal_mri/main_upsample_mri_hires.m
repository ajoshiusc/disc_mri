...
vout.hdr.dime.pixdim(3)=vout.hdr.dime.pixdim(3)/2.0;
vout.hdr.dime.dim(3)=2*size(v1.img,2);
vout.img=zeros(size(v1.img,1),size(v1.img,2)*2,size(v1.img,3));
...
for j = 1:size(v1.img,2)-1
    ...
    vout.img(:,2*j+1,:)=imresize((IW1+IW2)/2,size(v1.img,[1,3]));
j
end
save tmp1
