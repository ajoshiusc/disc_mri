...
def run_svr(subj, te, subdir, res=1, th=3, MAX_COMB=5):
    ...
    for num_stacks in range(1, len(stacks_all) + 1):
        for ns in range(MAX_COMB):
            ...
            cmd = (
                'docker run --cpus=12 --rm --mount type=bind,source=/deneb_disk/disc_mri/scan_11_3_2023_twins_nii_rot,target=/home/data fetalsvrtk/svrtk /bin/bash -c "cd /home/data; mirtk reconstruct '
                + outsvr
                + " "
                + str(num_stacks)
                + str_stacks
