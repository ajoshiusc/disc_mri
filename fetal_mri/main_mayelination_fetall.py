...
# write main function to calculate T1/T2 ratio
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        t1_t2_ratio = np.where(mask_data > 0.1, t1_data / t2_data, 0)
...
if __name__ == "__main__":
    ...
    t1list = os.listdir('/home/ajoshi/Downloads/fetal_brain_mri_atlas/structural/')
    t1list = [f for f in t1list if 't1-' in f and f.endswith('.nii.gz')]
    print("T1 files found:", t1list)

    for t1 in t1list:
        # Construct corresponding T2 and mask paths

        t1_path = os.path.join('/home/ajoshi/Downloads/fetal_brain_mri_atlas/structural/', t1)
        t2_path = t1_path.replace('t1-', 't2-')
        mask_path = t2_path
        ...
        # Define output path
        output_path = t1_path.replace('t1-', 't1_t2_ratio-').replace('.nii.gz', '.nii.gz')
