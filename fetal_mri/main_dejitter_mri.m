...
v1=load_untouch_nii('/home/ajoshi/Downloads/disc_data/fetal_mri/fetal_vol2_t2_haste_trufi/t2_haste_cor_TE119/_t2_haste_cor_TE119_20210915181625_9_bst.nii');%('/home/ajoshi/Desktop/tmp/haste.nii.gz');

v1.img = double(v1.img);

im_sz_x = size(v1.img,2);im_sz_y = size(v1.img,3);
...
for dejitter=1:10
    ...
    for j = 2:size(v1.img,1)-1
        ...
        Tx_full{j} = imresize(interp2(Tx_full{j},Tx,Ty),size(v1.img,2:3));
        Ty_full{j} = imresize(interp2(Ty_full{j},Tx,Ty),size(v1.img,2:3));
    ...
    save_untouch_nii_gz(vout,sprintf('out2_%d.nii.gz',dejitter));
