...
def load_comprehensive_results():
    ...
    for line in lines:
        ...
        if line.startswith('TE ') and line.endswith(' ms:'):
            ...
            data[current_te] = {
                'stacks': [], 'cr': [], 'cnr': [], 'snr_gm': [], 'snr_wm': [], 'ssim': [], 'nmse': []
            }
            continue
        ...
        if current_te is not None and '\t' in line:
            try:
                ...
                if len(parts) >= 8:
                    ...
                    data[current_te]['nmse'].append(nmse)
...
def create_publication_figure():
    ...
    # Plot 6: NMSE vs Stack Number
    ax6 = fig.add_subplot(gs[2, 1])
    for i, te_value in enumerate(TE_VALUES):
        if te_value in comprehensive_data:
            # Get all data points
            stacks = comprehensive_data[te_value]['stacks']
            mse_values = comprehensive_data[te_value]['nmse']
    ...
    # PNG version
    png_file = os.path.join(output_dir, "publication_figure_x_preserved.png")
