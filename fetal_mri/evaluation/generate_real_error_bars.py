...
def calculate_ssim_mse_metrics(img_data, reference_data):
    """Calculate SSIM and NMSE metrics for image quality assessment."""
    try:
        ...
        # Normalize for SSIM and NMSE
        img_valid = img_tensor[img_tensor > 0]
        ref_valid = ref_tensor[ref_tensor > 0]
        ...
        ref_norm = (ref_tensor - ref_min) / (ref_max - ref_min + 1e-8)
        ...
        ssim_loss_val = ssim_loss.forward(img_norm, ref_norm)
        ssim_value = 1.0 - ssim_loss_val.item()
        
        mse_value = mse_loss.forward(img_norm, ref_norm).item()
        nmse_value = mse_value / (ref_norm ** 2).mean().item()
        
        return ssim_value, nmse_value
        
    except Exception as e:
        print(f"Error calculating SSIM/NMSE: {e}")
        return np.nan, np.nan
...
def calculate_comprehensive_metrics(image_data, tissue_data, reference_data=None):
    ...
    ssim_val, nmse_val = np.nan, np.nan
    if reference_data is not None:
        ssim_val, nmse_val = calculate_ssim_mse_metrics(image_data, reference_data)
        
    return {
        'contrast_ratio': cr, 'cnr': cnr, 'snr_gm': snr_gm, 'snr_wm': snr_wm,
        'ssim': ssim_val, 'nmse': nmse_val
    }
...
def compute_real_error_bars_for_stack_count(te_value, target_stacks, svr_dir, tissue_data, reference_data, n_combinations=15):
        ...
        try:
            ...
            if not np.isnan(metrics_dict['contrast_ratio']):
                all_metrics.append([
                    ...
                    metrics_dict['ssim'], metrics_dict['nmse']
                ])
    ...
    if len(all_metrics) == 0:
        return {
            ...
            'snr_wm_mean': np.nan, 'snr_wm_std': np.nan,
            'ssim_mean': np.nan, 'ssim_std': np.nan,
            'nmse_mean': np.nan, 'nmse_std': np.nan
        }
    
    all_metrics = np.array(all_metrics)
    
    # Reject OUTLIER_REJECTION_RATE worst data as outliers per metric
    means = np.zeros(6)
    stds = np.zeros(6)
    ...
    for i in range(6):
        ...
        # NMSE is index 5 (lower is better, worst X% are highest values)
        # Others (indices 0-4) are higher is better (worst X% are lowest values)
        if i == 5:
            filtered = sorted(valid)[:n_keep]
        else:
            filtered = sorted(valid, reverse=True)[:n_keep]
            
        means[i] = np.mean(filtered)
        stds[i] = np.std(filtered)
    ...
    return {
        ...
        'nmse_mean': means[5], 'nmse_std': stds[5]
    }
    ...
    for te_value in TE_VALUES:
        ...
        for target_stacks in TARGET_STACK_NUMBERS:
            ...
            if not np.isnan(error_data['cr_mean']):
                ...
                print(f"    NMSE: {error_data['nmse_mean']:.2e}±{error_data['nmse_std']:.2e}")
            else:
