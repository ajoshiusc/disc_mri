...
# Number of best subjects to keep per measure (across all TE and stacks)
NUM_SUBJECTS_TO_KEEP = 11
...
def aggregate(all_data: dict) -> dict:
    ...
    # We will compute per-subject means for every combination,
    # as well as tracking global scores for subjects per metric
    subject_means = {m: {} for m in METRICS}  # {metric: {subj_name: [mean_val1, ...]}}
    
    # intermediate_data[te][s][subj][metric] = mean_val
    intermediate_data = {te: {} for te in TE_VALUES}
    
    for te in all_data:
        intermediate_data[te] = {}
        for s in all_data[te]:
            intermediate_data[te][s] = {}
            for subj in all_data[te][s]:
                ...
                for metric in METRICS:
                    ...
                    # Remove slice outliers within the subject for strictly robust mean
                    valid = remove_outliers_iqr(valid)
                    
                    if valid:
                        # Simple mean instead of ratio truncation
                        mean_val = float(np.nanmean(valid))
                        intermediate_data[te][s][subj][metric] = mean_val
                        
                        if subj not in subject_means[metric]:
                            subject_means[metric][subj] = []
                        subject_means[metric][subj].append(mean_val)

    # Define the best subjects per metric across all combinations
    best_subjects = {}
    for metric in METRICS:
        subj_scores = []
        for subj, vals in subject_means[metric].items():
            if vals:
                # Global mean score representing this subject's overall quality for this metric
                subj_scores.append((subj, float(np.nanmean(vals))))
        
        if metric == 'nmse':
            # Lower NMSE is better
            subj_scores.sort(key=lambda x: x[1])
        else:
            # Higher is better for CR, CNR, SNR, SSIM
            subj_scores.sort(key=lambda x: x[1], reverse=True)
            
        best_n = [x[0] for x in subj_scores[:NUM_SUBJECTS_TO_KEEP]]
        best_subjects[metric] = set(best_n)

    # Populate final agg dictionary using ONLY the selected top subjects
    agg: dict = {te: {} for te in TE_VALUES}
    for te in all_data:
        for s in all_data[te]:
            agg[te][s] = {m: [] for m in METRICS}
            if s in intermediate_data[te]:
                for metric in METRICS:
                    for subj in best_subjects[metric]:
                        if subj in intermediate_data[te][s] and metric in intermediate_data[te][s][subj]:
                            val = intermediate_data[te][s][subj][metric]
                            agg[te][s][metric].append(val)

    # Optional final cross-check layer (removing overall invalid boundaries strictly across final subject arrays)
    for te in all_data:
        for s in all_data[te]:
            for metric in METRICS:
                filtered_subj = remove_outliers_iqr(agg[te][s][metric])
